...
def update_strategies_with_anchor(file_path: str, r1_params: dict, other_regime_params: dict, anchor_name: str):
    ...
    전용 YAML 업데이트 함수. (스타일 복제 방식을 사용한 최종 안정화 버전)
    """
    yaml = YAML()
    yaml.preserve_quotes = True
    yaml.indent(mapping=2, sequence=4, offset=2)
    ...
    r1_key = "SpikeHunter_R1_BullStable"
    anchored_value = None

    # 1. R1 파라미터 업데이트 및 앵커 설정
    if r1_key in data['strategies']:
        new_vol_val = r1_params.pop('max_market_vol', None)
        if new_vol_val is not None:
            # --- [핵심 수정] ---
            # 1-1. 스타일 복제를 위한 '템플릿' 객체를 가져옵니다. (파일 내 다른 float 값)
            template_node = data['strategies'][r1_key]['target_r']
            
            # 1-2. 템플릿의 타입을 사용하여 새로운 값을 ruamel.yaml 객체로 생성합니다.
            #      이렇게 하면 소수점 서식 등 내부 정보가 그대로 복사됩니다.
            new_anchored_value = type(template_node)(new_vol_val)
            
            # 1-3. 이제 완전한 상태의 객체에 앵커를 설정합니다.
            new_anchored_value.yaml_set_anchor(anchor_name)
            
            # 1-4. 앵커가 설정된 객체를 R1의 max_market_vol에 최종 할당합니다.
            data['strategies'][r1_key]['max_market_vol'] = new_anchored_value
            anchored_value = new_anchored_value
            # --- [수정 완료] ---

        # 나머지 R1 파라미터 업데이트
        for key, value in r1_params.items():
            if key in data['strategies'][r1_key]:
                data['strategies'][r1_key][key] = value

    # 2. 다른 체제들(R2, R3)의 파라미터 업데이트
    for regime_key, params in other_regime_params.items():
        if regime_key in data['strategies']:
            for key, value in params.items():
                if key in data['strategies'][regime_key] and key != 'max_market_vol':
                    data['strategies'][regime_key][key] = value

    # 3. R2, R3, R4가 생성된 앵커 객체를 참조(별칭 사용)하도록 설정
    if anchored_value:
        for key in ["SpikeHunter_R2_BullVolatile", "SpikeHunter_R3_BearStable", "SpikeHunter_R4_BearStable"]:
            if key in data['strategies']:
                data['strategies'][key]['max_market_vol'] = anchored_value
