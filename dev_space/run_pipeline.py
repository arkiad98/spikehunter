...
def run_walk_forward_pipeline(settings_path: str, strategy_name: str):
    ...
    for i, (train_start, train_end, test_start, test_end) in enumerate(tqdm(wf_periods, desc="Walk-Forward Periods")):
        ...
        full_dataset_path = os.path.join(period_feat_dir, "dataset_v4.parquet")
        if not os.path.exists(full_dataset_path):
            utils_logger.logger.error("데이터셋 파일 생성 실패")
            continue
        ...
        train.run_classification_pipeline(settings_path=train_settings_path)
...
def main():
    ...
    while True:
        ...
        print("4. 진단 및 심층 분석 (Add-ons)")
        print("5. 실전 예측 (Prediction)")
        print("0. 종료")
        ...
        elif choice == '5': predict.run_predict(args.settings)
        elif choice == '0': 
            print("프로그램을 종료합니다.")
            break
        else:
